var DitV = DitV || {
    ...
    fallout: function(name, severity){
    if (!state.DitV.characters[name]){ return "Character '" + name + "' not registered."; }
    var chips = state.DitV.characters[name]['chips'];
    var chipCount = 0;
    var severityCount = 0;
    ...
    helpMsg += "fallout NAME SEVERITY   determine the fallout from a conflict\n";
    ...
    case "add":
        if (tokens.length <= 3){
        error = "The 'add' command requires at least one argument: character name";
        break;
        }
        error = DitV.addCharacter(tokens[2], tokens[3]);
        break;
    case "remove":
        if (tokens.length <= 2){
